...
SinglyLinkedListNode* removeNodes(SinglyLinkedListNode* listHead, int x) {
    SinglyLinkedListNode* retHead = listHead;
    SinglyLinkedListNode* curitr  = listHead;
    SinglyLinkedListNode* preitr  = nullptr;
    while (curitr != nullptr) {
    //     // cout<<(curitr == nullptr)<<endl;
        if(curitr->data > x){
            // delete this element
            // cout<<curitr->data<<endl;
            // cout<< "curdata = "<<curitr->data<<endl;
            if(curitr == retHead){
                curitr  = retHead->next;
                retHead = curitr;
                continue;
            }else{
                preitr->next = curitr->next;
                curitr = preitr; 
                // free(curitr);             
            }
        }
        preitr = curitr;
        curitr = curitr->next;//move to the next element
    }
    return retHead;
}
int main()
